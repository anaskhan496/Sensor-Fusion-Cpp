...
struct Lidar
{
	...
	double resoultion;
	double sderr;
	// this lidar object is telling what physical objects can the ray cast off of. The phy objects are cars geenrated from init highway
		...
		: cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0,0,2.6)
	{
		...
		// TODO:: increase number of layers to 8 to get higher resoultion pcd by increasing the number of vertical layers
		int numLayers = 8;
		// the steepest vertical angle
		double steepestAngle =  30.0*(-pi/180);
		double angleRange = 26.0*(pi/180);
		// TODO:: set to pi/64 to get higher resoultion pcd. Angular resolution around the z axis
