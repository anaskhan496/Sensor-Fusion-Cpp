...
int main(int argc, const char *argv[])
{
    ...
    int imgStepWidth = 1;  // FPS of 10 images per second
    int imgFillWidth = 4;  // no. of digits which make up the file index (e.g. img-0001.png)

    // object detection (load class names from file)
    ...
    vector<DataFrame> dataBuffer; // list of data frames which are held in memory at the same time (see datastructures.h)
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        DataFrame frame; //instantiating dataframe
        frame.cameraImg = img; //adding image to the frame
        dataBuffer.push_back(frame); //push it to the vector (suboptimal)
        ...
        if(bVis)
        {
            show3DObjects((dataBuffer.end()-1)->boundingBoxes, cv::Size(4.0, 20.0), cv::Size(1000, 800), true);
        }
        bVis = false;
