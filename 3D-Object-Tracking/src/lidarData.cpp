...
void showLidarTopview(std::vector<LidarPoint> &lidarPoints, cv::Size worldSize, cv::Size imageSize, bool bWait)
{
    ...
    for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it)
    {
        ...
        //scale using the ratio between the image and world co-ordinates
        int y = (-xw * imageSize.height / worldSize.height) + imageSize.height; //xw is in the sensor co-ordinate system. y and x are the top view image co-ordinate system
